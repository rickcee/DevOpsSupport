...
pipeline {
    agent none
    environment {
        uniquePipelineId = 'MVN_NG_RELEASE_1'
		vcsSelection = null;
		vcsType = null;
        repoId = -1;
        repoUrl = '';
        ...
        stage('DEV Release Process') {
            ...
            steps {
                script {
					...
					def mvnCmd = mvnBin + ' clean compile -P dev ' + mvnOpts + ' deploy -DaltDeploymentRepository=' + nexusUrl.content
					try {
						sh mvnCmd
					} catch (e1) {
						try {
		...
		stage('UAT Release Process') {
			...
			steps {
				script {
					...
					def mvnCmd = mvnBin + ' clean compile -P uat ' + mvnOpts + ' deploy -DaltDeploymentRepository=' + nexusUrl.content
					try {
						sh mvnCmd
					} catch (e1) {
						try {
		...
		stage('STAGE Release Process') {
			...
			steps {
				script {
					...
					def mvnCmd = mvnBin + ' clean compile -P stage ' + mvnOpts + ' deploy -DaltDeploymentRepository=' + nexusUrl.content
					try {
						sh mvnCmd
					} catch (e1) {
						try {
		...
		stage('PRODUCTION Release Process') {
			...
			steps {
				script {
					...
					def mvnCmd = mvnBin + ' clean compile -P prod ' + mvnOpts + ' deploy -DaltDeploymentRepository=' + nexusUrl.content
					try {
						sh mvnCmd
					} catch (e1) {
						try {
